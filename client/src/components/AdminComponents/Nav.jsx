...
const Nav = ({ show }) => {
    ...
    return (
      <aside className={`${show? 'left-0' : '-left-full'} top-0
       text-gray-500 p-4 fixed w-full bg-bgGray h-full md:static md:w-auto transition-all`}>
        ...
        <nav className="flex flex-col gap-2">
            ...
            <svg xmlns="http://www.w3.org/2000/svg" fill="none" viewBox="0 0 24 24" strokeWidth={1.5} stroke="currentColor" className={pathname.includes('/setting') ? activeIcon : inactiveIcon}>
            ...
            <button            
            onClick={() => signOut()} 
            className={`${inactiveLink} items-center `}>
              <IoLogOutOutline className="font-semibold text-2xl"/> Logout
            </button>
        </nav>
      </aside>
    );
